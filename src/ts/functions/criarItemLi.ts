export default function criarItemLi(nome: string, quantidade: number, id: number, side: string): string {
    return `
    <li class="${side} flex justify-between gap-3 border-b-2 lg:text-2xl lg:px-8" id="${id}">
        <div class="${side}-text flex items-center gap-2 overflow-auto w-4/5">
            <button id="btn-id-item-${id}" class="w-6 h-6 hidden"><img  src="./dist/image/btn-confirm-icon.png" alt="Confirmar"></button>
